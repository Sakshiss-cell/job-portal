...
export const getSampleJdJSON = async (filters) => {
  ...
  return allJobs.filter((job) => {
    ...
    return (
      ...
      (!filters.companyName || job.companyName.includes(filters.companyName)) &&
      (!filters.location || job.location === filters.location.toLowerCase()) &&
      (!filters.minBasePay || job.minJdSalary >= filters.minBasePay) &&
      (!filters.jobRole ||
        job.jobRole
          .toLocaleLowerCase()
          .includes(filters.jobRole.toLocaleLowerCase()))
    );
  });
};
